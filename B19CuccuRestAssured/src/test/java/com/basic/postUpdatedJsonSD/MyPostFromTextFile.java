...
public class MyPostFromTextFile {
	...
	public void user_hit_the_post_request_with_json_data(String username) throws IOException{
		...
		//Read data from file
		String data = ""; 
	    data = new String(Files.readAllBytes(Paths.get("C:\\Users\\admin\\gitCucuRestAssured\\CucuRestAssured\\src\\test\\java\\com\\basic\\postFromTextFileSD\\user.json"))); 
	  ...
	  //below code is used to update data by taking  put request and editing same by comparing id generated
	   // resp = RestAssured.given().accept(ContentType.JSON).relaxedHTTPSValidation().
			//	when().body(newJsonStr).put("https://reqres.in/api/users");
	}
