...
//		resp.then().log().status();
		...
		// resp.asstring is specific for rest assured
	...
	public void user_validate_the_id_is_not_null(){
		...
		//Check id is not null( matchers validation which comes with cucumber but we dont use )
		resp.then().assertThat().body("id", notNullValue());
		
		//check id is greater than zero (jason path validation we validate from here)
